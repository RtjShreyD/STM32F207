...
#define SDIO_CMD0TIMEOUT                ((uint32_t)0x00010000)//((uint32_t)0x00010000)//((uint32_t)0x00002710)
...
#define SDIO_INIT_CLK_DIV                  ((uint8_t)0x76)//((uint8_t)0xB2)
#define SDIO_TRANSFER_CLK_DIV              ((uint8_t)0x00) //((uint8_t)0x1) 
...
/**
 ...
 SD_Error SD_WriteBlock(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize)//(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize)
{
   SD_Error errorstatus = SD_OK;
   ...
   uint32_t bytestransferred = 0, count = 0, restwords = 0;
   uint32_t *tempbuff = (uint32_t *)writebuff;
   uint32_t status =0; // 20141211 S. Jung Add code
   ...
   while ((SDIO->STA & SDIO_FLAG_CMDACT) && SDIO_FLAG_CMDACT)
   ...
   if (SD_OK != errorstatus)
   {
   	 printf("SD Write Block 1error : %d\r\n",errorstatus);
     return(errorstatus);
   ...
   if (errorstatus != SD_OK)
   {
 	 printf("SD Write Block 2error : %d\r\n",errorstatus);
     return(errorstatus);
 ...
 #elif defined (SD_DMA_MODE)
   SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
   DMA_TxConfiguration((uint32_t *)writebuff, BlockSize);//SD_LowLevel_DMA_TxConfig((uint32_t *)writebuff, BlockSize);
   SDIO_DMACmd(ENABLE);
 #endif
